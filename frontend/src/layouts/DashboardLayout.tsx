  ...
  return (
    <div>
      <div className="fixed left-0 top-0 w-60 z-10 bg-primary-main h-[100vh] text-primary-contrast px-2">
        <Link to={'/dashboard'} className='text-2xl text-start pl-2 py-4 block'>LapasPanic</Link>
        <Divider />
        <div className="flex flex-col mt-2">
          ...
          <Collapse in={openCollapse} unmountOnExit className='pl-4'>
            <SidebarLinkButton linkTo='users' startIcon={<VisibilityIcon />} text='Daftar Pengguna' className='text-sm' />
            <SidebarLinkButton linkTo='users/create' startIcon={<AddIcon />} text='Tambah Pengguna' className='text-sm' />
          </Collapse>
      ...
      <div className="fixed top-0 h-16 bg-primary-main w-full flex justify-end items-center text-primary-contrast px-4">
        <Link to={'/'}>
          <Button variant='contained' sx={{ color: "white" }} color='primary' endIcon={<LogoutIcon />} >
            Logout
          </Button>
        </Link>
      </div>
